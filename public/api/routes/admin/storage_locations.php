...
function handle_list_locations(): void {
    ...
    $rows = db()->query(
        'SELECT id, name, description, qr_code, created_at,
...
function handle_create_location(): void {
    ...
    try {
        $stmt = db()->prepare(
            'INSERT INTO storage_locations (name, description, qr_code) VALUES (?, ?, ?)'
        );
        $stmt->execute([$name, $description ?: null, $qr_code]);
    ...
    json_ok(['id' => $id, 'name' => $name, 'description' => $description ?: null, 'qr_code' => $qr_code], 201);
}
...
function handle_update_location(): void {
    ...
    try {
        $stmt = db()->prepare(
            'UPDATE storage_locations SET name = ?, description = ? WHERE id = ?'
        );
